...
def worst_label(df: pd.DataFrame, fallback: str = None) -> str:
    ...
    nothing usable is found. (Kept for potential future use; not currently
    called since Summary Report now requires an explicit click at every
    level instead of auto-picking a default.)"""
...
def render_summary_report():
    """Progressive drill-down Summary Report, all on one page:
      1) Fail % by Line                         -- always shown
      2) Fail % by Sheet/Tab in the clicked line -- only appears after
         you click a bar in chart 1
      3) Fail % by Parameter in the clicked tab  -- only appears after
         you click a bar in chart 2
    Each chart is a brand-new graph generated by clicking the bar above it
    (nothing is pre-picked/auto-shown) — clicking a Parameter bar in chart 3
    then shows that parameter's detail chart below it.
    A single date-range filter at the top applies to every chart and to
    the detail view, so you can zoom into a specific time window.
    """
    init_state()
    ...
    if st.button("🔁 Reset (clear all selections)", key="btn_reset_focus"):
    ...
    event1 = st.plotly_chart(
        plot_fail_bar(df_lines, "Fail % by Line (click a bar to see its sheets/tabs)", "Line"),
    ...
    if not st.session_state.drill_line:
        st.info("👆 Click a bar above to see that line's sheets/tabs.")
        return

    current_line = st.session_state.drill_line
    st.caption(f"Selected line: **{current_line}**")
    spreadsheet_id = LINES[current_line]

    st.divider()

    # ---------------- 2) Fail % by Sheet/Tab (only after clicking a line) ----------------
    ...
    event2 = st.plotly_chart(
        plot_fail_bar(df_tabs, f"Fail % by Sheet/Tab in {current_line} (click a bar to see its parameters)", "Sheet / Tab"),
    ...
    if not st.session_state.drill_tab:
        st.info("👆 Click a bar above to see that sheet/tab's parameters.")
        return

    current_tab = st.session_state.drill_tab
    st.caption(f"Selected sheet/tab: **{current_tab}**")

    st.divider()

    # ---------------- 3) Fail % by Parameter (only after clicking a tab) ----------------
    ...
    # ---------------- Parameter detail (only after clicking chart 3) ----------------
    ...
    if st.session_state.drill_param_uid and st.session_state.drill_param_uid in label_by_uid:
        ...
        with st.expander("➕ Compare more parameters from this tab"):
            extra_labels = st.multiselect(
                ...
                [l for l in label_to_param if l != focused_label],
        ...
        st.info("👆 Click a bar above to see that parameter's detail chart here.")
